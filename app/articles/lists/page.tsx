          ...
          <div className="p-6">
              ...
              <Table className="w-full">
                <TableHeader>
                  <TableRow className="bg-gray-50">
                    {["ID", "Title", "Date", "Actions"].map((header, index) => (
                      <TableHead key={header} className="font-bold text-black text-lg py-4">
                        <Button
                          variant="ghost"
                          onClick={() => handleSort(header.toLowerCase() as SortableKeys)}
                          className="font-bold text-black hover:bg-gray-100"
                        >
                          {header} {index < 3 && <ArrowUpDown className="ml-2 h-4 w-4" />}
                        </Button>
                      </TableHead>
                    ))}
                  </TableRow>
                ...
                {paginatedArticles.map((article) => (
                  <TableRow key={article.id} className="hover:bg-gray-50 transition-colors">
                    <TableCell className="font-medium text-black">{article.id}</TableCell>
                    <TableCell className="text-black">{article.title}</TableCell>
                    <TableCell className="text-black">{formatDate(article.date)}</TableCell>
                    <TableCell>
                      <DropdownMenu>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0 mt-8 py-6 border-t border-gray-200">
              <div className="text-sm text-gray-600">
                ...
                {Math.min(currentPage * ITEMS_PER_PAGE, filteredArticles.length)} of {filteredArticles.length} entries
              </div>
