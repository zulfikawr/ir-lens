...
const NewsletterSubscription = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.errors?.email || 'Subscription failed');
      }

      setStatus("success");
      setEmail("");
      toast({
        description: "Thank you for subscribing! You'll receive updates when new articles are published.",
        duration: 3000,
      });
    } catch (error) {
      setStatus("error");
      const errorMsg = error instanceof Error ? error.message : "Subscription failed";
      ...
      <div className="w-full max-w-5xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Subscribe to our weekly newsletter for in-depth analysis of global affairs, 
            regional developments, and exclusive insights from diplomatic correspondents.
          </p>
        </div>
