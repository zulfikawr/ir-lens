  ...
  useEffect(() => {
    if (displayedArticles.length > 0) {
      const interval = setInterval(() => {
        setActiveCard((prev) => (prev + 1) % displayedArticles.length);
      }, 5000);
      ...
      {displayedArticles.map((article, index) => (
        <article
          ...
          className={`absolute w-full h-full transition-all duration-500 ease-in-out 
            ...
            ${index === (activeCard + 1) % displayedArticles.length ? 'translate-x-0 translate-y-0' : 'z-10'} 
            ${index === (activeCard + 2) % displayedArticles.length ? 'translate-x-2 translate-y-2 md:translate-x-4 md:translate-y-4' : 'z-20'}`}
        >
            ...
            <div className='flex flex-col p-4 md:w-1/2 h-3/5 md:h-full overflow-y-auto'>
              <div className='flex flex-col h-full'>
                ...
                <Link href={getArticleUrl(article)}>
                  <h2 className='text-2xl md:text-3xl lg:text-4xl xl:text-5xl md:line-clamp-5 font-bold leading-tight text-black hover:underline mb-2 md:mb-4'>
                    {article.title}
                  </h2>
                </Link>

                <p className='text-sm md:text-base text-gray-600 md:line-clamp-5 leading-relaxed mb-4 flex-grow overflow-y-auto'>
                  {article.description}
                </p>
