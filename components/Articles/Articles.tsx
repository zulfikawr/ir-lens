...
export default function ExperimentalNewspaper() {
  ...
  const totalPages = useMemo(
    () => Math.ceil(allArticles.length / articlesPerPage),
    [allArticles],
  );
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-6 h-full max-h-[468px] overflow-hidden'>
                  <div className='flex flex-col justify-between'>
                    <div>
                      ...
                      <p className='text-sm md:text-base text-neutral-600 line-clamp-4'>
                        {article.description}
                      </p>
                    </div>
          ...
          <div className='bg-white border border-neutral-200 p-6 shadow-md rounded-lg'>
            ...
            <ul className='space-y-4'>
              {allArticles.slice(0, 5).map((article, index) => (
                <li
                  key={article.title}
                  className='pb-4 border-b last:border-b-0 border-neutral-200'
                >
                  <Link
                    href={`/articles/${article.slug}`}
                    className='group flex items-center'
                  >
                    <span className='text-2xl font-thin text-neutral-300 mr-4'>
                      {index + 1}
                    </span>
                    <div>
