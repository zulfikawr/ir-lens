...
import { Share2, Twitter, Instagram, Mail, Link as LinkIcon, Copy } from "lucide-react";
import { useToast } from "@/hooks/use-toast"
import { ArticleType } from '@/types/article';
import Image from 'next/image';

export const ArticleShareDialog = ({ article }: { article: ArticleType["articles"][0] }) => {
      ...
      <DialogContent className="sm:max-w-2xl border-black p-0 overflow-hidden">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="w-full md:w-1/2 p-4 md:p-6 space-y-4">
            <DialogHeader>
              <DialogTitle className="text-left text-2xl font-bold">{article.title}</DialogTitle>
